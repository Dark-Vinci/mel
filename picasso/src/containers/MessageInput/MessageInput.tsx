    ...
    switch (format.type) {
      case 'text': {
        if (selectedText) {
          ...
          const template =
            format.template || format.prefix + 'text' + format.suffix;
      ...
      case 'line': {
        if (selectedText) {
          ...
          newText =
            message.slice(0, start) + newSelectedText + message.slice(end);
          newStart = start;
          newEnd = start + newSelectedText.length;
        } else {
          newText =
            message.slice(0, start) + format.prefix + message.slice(end);
          newStart = start + format.prefix.length;
          newEnd = newStart;
        }
      ...
      case 'block': {
        if (selectedText) {
          ...
          newText =
            message.slice(0, start) + format.template + message.slice(end);
          newStart = start + 4;
          newEnd = newStart + 4;
        }
    ...
    <div>
      <div>
        <div>
          ...
          <button
            onClick={() => applyFormat('bulletedList')}
            style={buttonStyle}
          >
            •
          </button>
        ...
        <div>
          <div>
            <div>
              ...
              <div onClick={() => setShowFormatting(!showFormatting)}>Aa</div>
          ...
          <div>
            <div>
              <Send shouldActivate={message.length > 0} />
            </div>
