...
use chrono::Utc;
use sea_orm::{ActiveModelTrait, DbErr, EntityTrait, IntoActiveModel, PaginatorTrait, QuerySelect};
use sea_orm::ActiveValue::Set;
use tracing::{debug, error};
use uuid::Uuid;
use sdk::errors::RepoError;
use sdk::models::db::messaging::message::{ActiveModel, Model as Message, Entity as MessageEntity};
use sdk::models::others::messaging::{CreateMessage, UpdateMessage};
...
pub trait MessageRepository {
    async fn create(&self, payload: CreateMessage, request_id: Uuid) -> Result<Message, RepoError>;
    async fn update(&self, payload: UpdateMessage, request_id: Uuid) -> Result<Message, RepoError>;
    async fn delete(&self, id: Uuid, request_id: Uuid) -> Result<(), RepoError>;
    async fn get_by_id(&self, id: Uuid, request_id: Uuid) -> Result<Message, RepoError>;
    async fn get_for_channel(&self, channel_id: Uuid, pagination: Pagination, request_id: Uuid) -> Result<Paginated<Vec<Message>>, RepoError>;
}
...
impl MessageRepository for MessageRepo {
    ...
    async fn create(&self, payload: CreateMessage, request_id: Uuid) -> Result<Message, RepoError> {
        debug!("Got Create Message request with request id {} and payload", request_id, payload);
    ...
    async fn update(&self, payload: UpdateMessage, request_id: Uuid) -> Result<Message, RepoError> {
        debug!("Got Update Message request with request id {} and payload", request_id, payload);
    ...
    async fn delete(&self, id: Uuid, request_id: Uuid) -> Result<(), RepoError> {
        debug!("Got Delete Message request with request id {} and id {}", request_id, id);

        let mut message = self.get_by_id(id, request_id).await?.into_active_model();
    ...
    async fn get_by_id(&self, id: Uuid, request_id: Uuid) -> Result<Message, RepoError> {
        debug!("Got Get Message request with request id {} and id {}", request_id, id);
    ...
    async fn get_for_channel(&self, channel_id: Uuid, pagination: Pagination, request_id: Uuid) -> Result<Paginated<Vec<Message>>, RepoError> {
        debug!("Got Get For Channel request with request id {} and id {}", request_id, channel_id);
        ...
        let paginated = Paginated::new(result, pagination.total_pages(count), 0, 0, count);
